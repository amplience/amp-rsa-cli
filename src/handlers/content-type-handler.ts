...
import { CodecType, CodecTypes, getCodecs, getContentType, getContentTypeSchema, paginator } from "@amplience/dc-demostore-integration"
...
export class ContentTypeHandler extends CleanableResourceHandler {
    ...
    async import(context: ImportContext): Promise<any> {
        ...
        await installTypes(context, getCodecs(CodecTypes.commerce).map(getContentType))
        ...
        await installTypes(context, _.filter(Object.values(jsonTypes), s => !_.includes(_.map(getCodecs(CodecTypes.commerce), 'schema.uri'), s.contentTypeUri)))
